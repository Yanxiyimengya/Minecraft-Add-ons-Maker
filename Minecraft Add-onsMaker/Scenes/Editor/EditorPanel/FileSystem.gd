...
func _ready() : 
	res_tree.gui_input.connect(func(event : InputEvent) : 
		if (event is InputEventMouseButton && event.double_click && event.button_index == MOUSE_BUTTON_LEFT) : 
			var selected_item : TreeItem = res_tree.get_item_at_position(event.position);
			selected_item.set_editable(0, true);
			res_tree.set_selected(selected_item, 0);
			res_tree.edit_selected();
			get_viewport().set_input_as_handled();
		elif (event is InputEventKey && event.is_pressed() && event.keycode == KEY_DELETE) : 
		...
		match(id) : 
			...
			7 : 
				var file_path : String = ProjectManager.current_project_config.project_path + \
						"/res" + item.get_metadata(0).get_metadata(0).ascription.get_parent().get_metadata(0).path;
...
func _on_tree_item_edited() : 
	var edit_item : TreeItem = res_tree.get_selected();
